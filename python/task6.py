...
line = raw_input()
minus_values = map(int, line.split())

line = raw_input()
plus_values = map(int, line.split())
...
for m in range(M):
  bestvalue = 0
  for n in range(N):
    delta = plus_values[m] - minus_values[n]
    if delta > bestvalue:
      bestvalue = delta
  items.append((bestvalue, 1))


# for x in product(range(M), range(N)):
#   print x

# for customer in range(M):
  

print knapsack(items, N)[0]